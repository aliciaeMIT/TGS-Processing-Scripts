...
f.seek(0)
f.readline()
for i in xrange(lengthCOM):             # Read in the COM-5570.txt data
...
h.seek(0)
h.readline()
for i in xrange(lengthTEMP):            # Read in the TEMP-5570.txt data
...
if diff_sig_plot_minus_decay:       # Positive signal minus negative Signal minus decay
    ...
    plt.xlim(0.38,0.435)
    plt.grid()
    plt.savefig('AcousDampFit-5570.png')
    # plt.show()
...
if Transform_plot:      # Fourier Transform
    ...
    plt.grid()
    # plt.show()
    plt.savefig('Transform-5570.png')
