...
for i=1:length(temprange)
    for j=0:2
        ...
        [freq,freq_err,speed,diff,diff_err,tau(:,3*(i-1)+j+1),tauErr, A, AErr, beta, betaErr, B, BErr, theta, thetaErr, C, CErr, file_date_time]=TGSPhaseAnalysis(pos_str,neg_str,grat,2,0, baselineBool, POSbaselineStr, NEGbaselineStr);
        
        fid1 = fopen( printFile, 'a' );
        fprintf(fid1, string('\n%s %s %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g'), pos_str, file_date_time, grat, freq, freq_err, A, AErr, diff, diff_err, beta, betaErr, B, BErr, theta, thetaErr, tau(3,3*(i-1)+j+1), tauErr, C, CErr);
        fclose(fid1);
    end
end
close all
