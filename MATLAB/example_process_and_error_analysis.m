%Inputs go here
calibration_filename_base = 'example_data\irradiated_tungsten\Tungsten_Calibration-2022-01-13-06.40um-spot00';
pname = 'example_data\irradiated_tungsten'; %ADJUST
...
%     end
    ...
    [freq(jj),freq_err(jj),speed(jj),diff(jj),diff_err(jj),tau(:,jj),tauErr(jj), A(jj), AErr(jj), beta(jj), betaErr(jj), B(jj), BErr(jj), theta(jj), thetaErr(jj), C(jj), CErr(jj)]=TGSPhaseAnalysis(pos_str,neg_str,grat,2,0,baselineBool,POSbaselineStr,NEGbaselineStr);
end
