...
% Calibrated grating spacings - this script ran through a bunch of nominal
% grating spacings for tungsten to check depth effects. gratArr was changed
% each time.
...
for i=1:length(gratArr)
    ...
    close all
    pos_str=strcat(pname,string('\'),str_base,num2str(nominalSpacing,formatSpec),'0um-spot',num2str(mod(i-1,3)),'-POS-1.txt');
    neg_str=strcat(pname,string('\'),str_base,num2str(nominalSpacing,formatSpec),'0um-spot',num2str(mod(i-1,3)),'-NEG-1.txt');
    ...
    [freq,freq_err,speed,diff,diff_err,tau(:,i),tauErr, A, AErr, beta, betaErr, B, BErr, theta, thetaErr, C, CErr, file_date_time]=TGSPhaseAnalysis(pos_str,neg_str,gratArr(i),2,0,1, POSbaselineStr, NEGbaselineStr);
    
    fid1 = fopen( printFile, 'a' );
    fprintf(fid1, string('\n%s %s %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g'), pos_str, file_date_time, gratArr(i), freq, freq_err, A, AErr, diff, diff_err, beta, betaErr, B, BErr, theta, thetaErr, tau(3,3*(i)), tauErr, C, CErr);
    fclose(fid1);
