...
% directory location
pname = 'C:\Users\apcwy\OneDrive - Massachusetts Institute of Technology\MIT_postdoc\TGS\Data\Kyocera_W_W\Tungsten_Kyocera\2024-02-05'; %ADJUST
...
for j=1:numberOfRuns
    ...
    [freq,freq_err,speed,diff,diff_err,tau(:,3*(j)),tauErr, A, AErr, beta, betaErr, B, BErr, theta, thetaErr, C, CErr]=TGSPhaseAnalysis(pos_str,neg_str,grat,2,0);
    
    %print these to the postprocessing file
    fid1 = fopen( printFile, 'a' );
    fprintf(fid1, string('\n%s %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g'), pos_str, grat, freq, freq_err, A, AErr, diff, diff_err, beta, betaErr, B, BErr, theta, thetaErr, tau(3,3*(j)), tauErr, C, CErr);
    fclose(fid1);
end
