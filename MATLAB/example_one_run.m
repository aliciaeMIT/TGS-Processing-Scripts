...
[freq_final,freq_error,speed,diffusivity,diffusivity_err,tau, tauErr, ParamA, AErr, ParamBeta, BetaErr, paramB, BErr, paramTheta, thetaErr, paramC, CErr, file_date_time] = TGSPhaseAnalysis(posstr,negstr,grat,2,0,blbool,baselinePOS, baselineNEG);
disp('Frequency = ');
disp(freq_final);
...
disp('File date and time = ');
disp(file_date_time);

fid1 = fopen( printFile, 'a' );
    naming_str=strcat(str_base,num2str(i));
    fprintf(fid1, string('\n%s %s %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g'), naming_str, file_date_time, grat, freq_final, freq_error, ParamA, AErr, diffusivity, diffusivity_err, ParamBeta, BetaErr, paramB, BErr, paramTheta, thetaErr, tau(3), tauErr, paramC, CErr);
    fclose(fid1);
