...
pos_aux = readvars(pos_file,"NumHeaderLines",16); %[[[Maybe roll this in when the first total_signal is created to minimise the number of times that the file is called]]]
...
pos=dlmread(pos_file,'',header_length,0);
neg=dlmread(neg_file,'',header_length,0);
if baselineBool
    posbas=dlmread(POSbaselineStr,'',header_length,0);
    negbas=dlmread(NEGbaselineStr,'',header_length,0);
end
...
%               %%%%%%plot vertical line at start time
                ...
                if ~steel
                    ...
                    saveas(gcf,strcat(pos_fpath,"/plots/",pos_fname,"_TGS_Final_Fit.png"))
                end
            end
        end
